 ...
 * ## 使用方式（典型场景）
 * @code
 *   bakuon::core::Container registry;
 *
 ...
 *   auto conn = registry.onConstruct<Selected>([](Container&, Entity entity) {
 *       // ... 通知 gui 层 entity 对应的界面元素需要高亮
 *   });
 *
 ...
 * 本类是内部实现（b_ 前缀），但由于 Container 的公开接口全是模板（EnTT 本身就是
 * header-only 模板库），没有跨动态库边界的符号导出问题——core 目前也仍然是
 * STATIC 库，不像 gui 那样需要 BAKUON_GUI_EXPORT。include/bakuon/core/Container.h
...
public:
    ...
    //    Slot::owner），一旦 Container 被移动，所有已经存在的订阅（包括尚未触发过
    ...
    //    构造 Container"的场景请用 std::unique_ptr<Container> 或 std::optional<Container>
    //    间接持有，而不是依赖 Container 自身的移动语义。
    ...
    /// 销毁一个实体及其携带的全部组件；对已经无效的 entity 是安全的空操作。
    ...
    /// 该 entity 当前是否指向一个存活的实体（默认构造的 Entity 恒返回 false）。
     ...
     *       如本文件顶部 Container 类文档里的 Selected 例子）做了专门优化，这类组件
    ...
    {
        return m_registry.template all_of<Ts...>(entity);
    }

    template<typename... Ts>
    [[nodiscard]] bool hasAny(Entity entity) const
    {
        return m_registry.template any_of<Ts...>(entity);
    }

    template<typename T, typename... Args>
    [[nodiscard]] decltype(auto) getOrEmplace(Entity entity, Args&&... args)
    {
    ...
    {
        return m_registry.template get<Ts...>(entity);
    }
    ...
    /// 取组件指针；不存在（或 entity 本身无效）时返回 nullptr，不抛异常、不断言。
    template<typename... Ts>
    [[nodiscard]] auto tryGet(Entity entity)
    {
        return m_registry.template try_get<Ts...>(entity);
    }
    template<typename... Ts>
    ...
    {
        m_registry.patch<T>(entity, std::forward<Func>(func));
    }
    ...
    void each(Func&& func)
    {
        m_registry.view<entt::entity>().each(
            [&func](const entt::entity entity) { std::invoke(func, Entity{entity}); });
    }
     ...
     * @param handler 签名为 void(Container&, Entity)；回调触发时组件已经挂载完毕，
    ...
    /// 前置声明；完整定义在本文件末尾（class Container::BatchGuard），因为它
    /// 需要调用 beginBatch()/endBatch()，必须等 Container 的完整定义之后才能写。
    class BatchGuard;
    ...
    /// 使用本类已封装的接口，不要绕开它直接操作 registry()——那样会让"core 不依赖
...
private:
    ...
    // 不立即触发 callback，而是把 Entity 记进 pending（去重），并且只在"本 Slot
    // 本批次第一次"产生 pending 项时，把自己的 flush() 登记进 Container 的
    // 待冲洗列表——避免每次事件都重复登记同一个 Slot。
    template<typename T>
    struct Slot : std::enable_shared_from_this<Slot<T>>
    {
        std::function<void(Container&, Entity)> callback;
        Container* owner = nullptr;
        std::vector<Entity> pending;  // 当前批次内已经去重的待通知列表
        ...
        void invoke(entt::registry&, entt::entity entity)
        {
            ...
            if (!flushRegistered) {
                ...
                // disconnect() 内部同步被销毁，而这里登记进 Container 的 flush
        ...
        void flush()
        {
            ...
            std::vector<Entity> toNotify;
            toNotify.swap(pending);
            if (!callback) {
                return;
            }
            for (Entity id : toNotify) {
                callback(*owner, id);
            }
        }
    };
    ...
    [[nodiscard]] Connection bindSink(Sink sink, Handler&& handler)
    {
        ...
        // dismiss 路径：不断开、也不释放，而是把 slot 的所有权交给 Container
        // 自己保管（m_pinnedSlots），让它至少活到 Container 析构为止——
 ...
 * @code
 *   {
 *       Container::BatchGuard batch(registry);
 ...
 * @endcode
 */
class Container::BatchGuard
{
public:
    explicit BatchGuard(Container& registry) noexcept
        : m_registry(&registry)
    {
