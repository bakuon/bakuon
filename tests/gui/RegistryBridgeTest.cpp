...
TEST(RegistryBridgeTest, RemoveEmitsEntityDestroyed)
{
    ...
    container.remove<Position>(node);
...
TEST(RegistryBridgeTest, MultipleWatchedTypesUseDistinctTags)
{
    ...
    const core::Entity node = container.create();
    container.emplace<Position>(node, 0.f, 0.f);
    container.emplace<Selected>(node);
...
TEST(RegistryBridgeTest, UnwatchedComponentTypesProduceNoSignal)
{
    ...
    QSignalSpy spy(&bridge, &gui::RegistryBridge::entityConstructed);
    const core::Entity node = container.create();
    container.emplace<Selected>(node); // 没有 watch<Selected>()，不应该产生任何信号
...
TEST(RegistryBridgeTest, DestroyingBridgeDisconnectsFromRegistry)
{
    ...
    // bridge 已经不存在了，但 container 应该仍然可以正常工作，不应该崩溃。
    EXPECT_NO_THROW(container.patch<Position>(node, [](Position& p) { p.x = 1.f; }));
    EXPECT_EQ(container.tryGet<Position>(node)->x, 1.f);
}

TEST(RegistryBridgeTest, BatchedUpdatesStillProduceOneSignalPerCoalescedNotification)
{
    // RegistryBridge 只是把 core::Container 已经产生的通知转成信号，Registry 自身
    ...
    QSignalSpy spy(&bridge, &gui::RegistryBridge::entityUpdated);
    {
        core::Container::BatchGuard batch(container);
        container.patch<Position>(node, [](Position& p) { p.x = 1.f; });
        container.patch<Position>(node, [](Position& p) { p.x = 2.f; });
        container.patch<Position>(node, [](Position& p) { p.x = 3.f; });
    }
