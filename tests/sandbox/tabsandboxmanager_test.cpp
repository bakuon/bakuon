...
TEST(TabSandboxManagerTest, OpenRunCloseLifecycle)
{
    ...
    TabId tabId;
    QObject::connect(&manager, &TabSandboxManager::tabLaunching, &manager,
                      [&](TabId id) {
    ...
    QObject::connect(&manager, &TabSandboxManager::tabRunning, &manager, [&](TabId id) {
        if (id == tabId) {
            observedForTab.push_back(TabState::Running);
        }
    });
    bool closed = false;
    QObject::connect(&manager, &TabSandboxManager::tabClosed, &manager, [&](TabId id) {
        if (id == tabId) {
            closed = true;
        }
    });
...
TEST(TabSandboxManagerTest, ConcurrencyThrottlingBackfillsQueue)
{
    ...
    std::vector<TabId> queuedIds;
    QObject::connect(&manager, &TabSandboxManager::tabQueued, &manager,
                      [&](TabId id) { queuedIds.push_back(id); });

    const TabId first  = manager.openTab(sandboxedExamplePluginPath());
    const TabId second = manager.openTab(sandboxedExamplePluginPath());
    ...
    ASSERT_TRUE(waitUntil([&] { return manager.tabState(second) == TabState::Launching ||
...
TEST(TabSandboxManagerTest, FaultedTabDoesNotAffectOtherTabs)
{
    ...
    QString faultReason;
    TabId faultedTabId;
    QObject::connect(&manager, &TabSandboxManager::tabFaulted, &manager,
                      [&](TabId id, const QString &reason) {
                          faultedTabId = id;
                          faultReason  = reason;
                      });
    ...
    const TabId badTab  = manager.openTab(QStringLiteral("/nonexistent/path/does_not_exist.so"));
    const TabId goodTab = manager.openTab(sandboxedExamplePluginPath());
    ASSERT_TRUE(badTab.isValid());
    ASSERT_TRUE(goodTab.isValid());
...
TEST(TabSandboxManagerTest, RestartTabReplacesSandboxId)
{
    ...
    const TabId tabId = manager.openTab(sandboxedExamplePluginPath());
    ASSERT_TRUE(tabId.isValid());
    ASSERT_TRUE(waitUntil([&] { return manager.tabState(tabId) == TabState::Running; }));
